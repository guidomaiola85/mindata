...
public class HeroesServiceTest {
    ...
    public void testFindByIdShouldReturnHeroSuccesfull() {
        ...
        given(heroesService.findById(1)).willReturn(new Hero(1,HeroesUtils.SUPERMAN));

        Hero heroe = heroesService.findById(1);
        
		assertEquals(heroe.getId(),1);
		assertEquals(heroe.getName(),HeroesUtils.SUPERMAN);

    }

	
    @Test(expected = NoSuchElementException.class)
    public void testFindByInvalidIdhouldThrowException() {
